...
fn main() {
    let path = PathBuf::from(
        "/home/j0k3r/navigation-agent-mcp/crates/navigation-engine/src/capabilities/trace_flow.rs",
    );
    ...
    for needle in [
        ...
        if let Some(node) = find_smallest_containing(tree.root_node(), source.as_bytes(), needle) {
            ...
            for (depth, item) in chain.into_iter().take(6).enumerate() {
                println!("LEVEL {depth}");
