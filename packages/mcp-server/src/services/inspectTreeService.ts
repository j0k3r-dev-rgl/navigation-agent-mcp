...
export interface InspectTreeService {
  ...
  ): Promise<ResponseEnvelope<InspectTreeData>>;
}
...
export function createInspectTreeService(options: {
  ...
  return {
    async execute(input) {
      let response;
      try {
        response = await options.engineClient.request<InspectTreeEngineResult>({
          id: nextRequestId(),
...
function mapInspectTreeResult(result: InspectTreeEngineResult): InspectTreeData {
  return {
    root: result.root,
...
function buildSummary(root: string, entryCount: number, truncated: boolean): string {
  if (entryCount === 0) {
